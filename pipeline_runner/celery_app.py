...
celery_app = Celery("imtr-pipeline-runner", broker=app_config[CELERY_BROKER_URL], backend=app_config[CELERY_RESULT_BACKEND])
celery_app.conf.update(app_config)
celery_app.autodiscover_tasks(packages=["pipeline_runner.tasks.celery_tasks"], force=True)
